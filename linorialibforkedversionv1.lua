--=============================================================================
-- LINORIA LIBRARY | Fixed Edition | Primordial Glass
--=============================================================================
...
ScreenGui.Name = "LinoriaUI_" .. tostring(math.random(1, 999999));
ScreenGui.ResetOnSpawn = false;
ScreenGui.IgnoreGuiInset = true;
ScreenGui.DisplayOrder = 2147483640;

-- SEPARATE HANDLE GUI (on top, not clipped)
local HandleGui = Instance.new('ScreenGui');
HandleGui.Name = "LinoriaHandle";
HandleGui.ZIndexBehavior = Enum.ZIndexBehavior.Global;
HandleGui.ResetOnSpawn = false;
HandleGui.IgnoreGuiInset = true;
HandleGui.DisplayOrder = 2147483646;
ProtectGui(HandleGui);
HandleGui.Parent = CoreGui;
...
local Library = {
    ...
    SuccessColor = Color3.fromRGB(70, 220, 130);
...
function Library:SetBlur(Size)
    ...
    if Library.Toggled and Library.UseBlur then Library.BlurEffect.Size = Library.BlurSize; end
end

function Library:AddBlurSlider(Groupbox)
    Groupbox:AddToggle('LinoriaUseBlur', {
        Text = 'Background Blur'; Default = Library.UseBlur;
        Tooltip = 'Blur the background when the menu is open';
        Callback = function(Value) Library.UseBlur = Value; Library:UpdateBlur(); end;
    });
    Groupbox:AddSlider('LinoriaBlurSize', {
        Text = 'Blur Amount'; Default = Library.BlurSize;
...
function Library:AddDarkenSlider(Groupbox)
    Groupbox:AddToggle('LinoriaUseDarken', {
        Text = 'Background Darken'; Default = Library.UseDarken;
        Tooltip = 'Darken the background when the menu is open';
        Callback = function(Value) Library.UseDarken = Value; Library:UpdateBlur(); end;
    });
    Groupbox:AddSlider('LinoriaDarkenAmount', {
        Text = 'Darken Amount'; Default = Library.DarkenAmount;
        Min = 0; Max = 100; Rounding = 0; Suffix = '%';
        Callback = function(Value) Library.DarkenAmount = Value; Library:UpdateBlur(); end;
    });
end
...
function Library:SetKeybindTransparency(Value)
    ...
    if Library.KeybindColorFrame then Library.KeybindColorFrame.BackgroundTransparency = Value; end
end

function Library:AddKeybindTransparencySlider(Groupbox)
    Groupbox:AddSlider('LinoriaKeybindTransparency', {
        Text = 'Keybind Transparency'; Default = 0;
        Min = 0; Max = 100; Rounding = 0; Suffix = '%';
        ...
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            ...
            EndedConn = InputService.InputEnded:Connect(function(EndInput)
                if EndInput == Input or EndInput.UserInputType == Enum.UserInputType.Touch then
                    ...
                    if Library._ResizeHandle then
                        Library._ResizeHandle:Reposition()
                    end
                end
            end)
        end
...
function Library:GetLighterColor(Color, Mult)
    Mult = Mult or 1.5;
    local H, S, V = Color3.toHSV(Color);
    return Color3.fromHSV(H, S, math.min(1, V * Mult));
end;

function Library:InvertColor(Color)
    return Color3.new(1 - Color.R, 1 - Color.G, 1 - Color.B);
end;

function Library:AverageColors(...)
    local Colors = { ... };
    local R, G, B = 0, 0, 0;
    for _, c in next, Colors do R = R + c.R; G = G + c.G; B = B + c.B; end
...
function Library:Trim(Str) return (Str:gsub("^%s*(.-)%s*$", "%1")); end
function Library:Split(Str, Delim)
    local Result = {};
    for Token in Str:gmatch("([^" .. Delim .. "]+)") do table.insert(Result, Token) end
    return Result;
end
function Library:StartsWith(Str, Prefix) return Str:sub(1, #Prefix) == Prefix; end
function Library:EndsWith(Str, Suffix) return Str:sub(-#Suffix) == Suffix; end
function Library:Capitalize(Str) return Str:sub(1, 1):upper() .. Str:sub(2):lower(); end
function Library:Reverse(Str)
    local Result = "";
    for i = #Str, 1, -1 do Result = Result .. Str:sub(i, i); end
    return Result;
end
function Library:CountOccurrences(Str, Sub)
    local Count = 0;
    for _ in Str:gmatch(Sub) do Count = Count + 1; end
    return Count;
end

function Library:TableContains(t, Value)
    for _, v in next, t do if v == Value then return true end end
    return false;
end
function Library:TableCount(t)
    local Count = 0;
    for _ in next, t do Count = Count + 1 end
    return Count;
end
function Library:TableCopy(t)
    local Copy = {};
    for k, v in next, t do Copy[k] = v; end
    return Copy;
end
function Library:TableMerge(t1, t2)
    local Result = Library:TableCopy(t1);
    for k, v in next, t2 do Result[k] = v; end
    return Result;
end
function Library:TableFind(t, Value)
    for i, v in next, t do if v == Value then return i end end
    return nil;
end
function Library:TableReverse(t)
    local Result = {};
    for i = #t, 1, -1 do table.insert(Result, t[i]) end
    return Result;
end
function Library:TableShuffle(t)
    local Result = Library:TableCopy(t);
    for i = #Result, 2, -1 do
...
function Library:Round(Value, Precision)
    Precision = Precision or 0;
    if Precision == 0 then return math.floor(Value + 0.5) end
    return tonumber(string.format('%.' .. Precision .. 'f', Value));
end
function Library:Clamp(Value, Min, Max) return math.clamp(Value, Min, Max); end
function Library:Lerp(A, B, Alpha) return A + (B - A) * Alpha; end
function Library:Distance(x1, y1, x2, y2) return math.sqrt((x2 - x1)^2 + (y2 - y1)^2); end
function Library:Distance3D(v1, v2) return (v1 - v2).Magnitude; end
...
function Library:Unload()
    ...
    if Library._StudGui then pcall(function() Library._StudGui:Destroy() end); Library._StudGui = nil end
    if Library._AutoExecConn then pcall(function() Library._AutoExecConn:Disconnect() end); Library._AutoExecConn = nil end
    if HandleGui then HandleGui:Destroy() end
    ScreenGui:Destroy()
end
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        function KeyPicker:Update()
            ...
            if kbMode == 'Active' then KeybindEntry.Visible = State == true
            elseif kbMode == 'Toggled' then
                local parentOn = false
                if ParentObj and ParentObj.Type == 'Toggle' then parentOn = ParentObj.Value == true
                elseif KeyPicker.SyncToggleState and ParentObj then parentOn = ParentObj.Value == true
                else parentOn = true end
                KeybindEntry.Visible = parentOn
            else KeybindEntry.Visible = true end
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function ProcessButtonParams(Class, Obj, ...)
            ...
            if type(Obj.Func) ~= 'function' then Obj.Func = function() end end
        end
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        SliderInner.InputBegan:Connect(function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and not Library:MouseIsOverOpenedFrame() then
                ...
                local ChangedConn = InputService.InputChanged:Connect(function(Change)
                    if Change.UserInputType == Enum.UserInputType.MouseMovement or Change == Input then UpdateSlider(Change.Position.X) end
                end)
                local EndedConn
                EndedConn = InputService.InputEnded:Connect(function(EndInput)
                    if EndInput == Input or EndInput.UserInputType == Enum.UserInputType.Touch then
...
do
    ...
    function Library:ConfigureNotifications(Cfg)
        ...
        if Layout then Layout.VerticalAlignment = VAlign; Layout.HorizontalAlignment = HAlign; end;
    end;
...
function Library:SpawnNotify(Text, Time)
    local xw = (Library:GetTextBounds(Text, Library.Font, 13) or 200);
    local H = 22;
...
--=============================================================================
-- RESIZE HANDLE (separate overlay, not clipped)
--=============================================================================
local function BuildResizeHandle(Outer, MinW, MinH)
    ...
    local function Reposition()
        if Outer.Visible then
            ...
            Circle.Position = UDim2.fromOffset(absPos.X + absSize.X - 8, absPos.Y + absSize.Y - 8)
        else
            Circle.Visible = false
        end
    end
    Library._ResizeHandle = { Reposition = Reposition, Circle = Circle }

    Outer:GetPropertyChangedSignal('AbsolutePosition'):Connect(Reposition)
    Outer:GetPropertyChangedSignal('AbsoluteSize'):Connect(Reposition)
    Outer:GetPropertyChangedSignal('Visible'):Connect(Reposition)
    task.spawn(function()
    ...
    InputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            local newX = math.max(MinW, startSize.X.Offset + delta.X)
            local newY = math.max(MinH, startSize.Y.Offset + delta.Y)
            Outer.Size = UDim2.new(startSize.X.Scale, newX, startSize.Y.Scale, newY)
        end
    end)
...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(620, 620) end
    ...
    local Outer = Library:Create('Frame', {
        ...
        BackgroundColor3 = Library.BackgroundColor;
        BackgroundTransparency = Library.GlassEnabled and Library.OuterGlassTransparency or 0;
        BorderSizePixel = 0;
    ...
    local GlassStroke = Library:Create('UIStroke', { Color = Library.OutlineColor; Transparency = 0.35; Thickness = 1; Parent = Outer; });
    ...
    Library:MakeDraggable(TitleBar, 28, true)
    -- Also allow dragging from the Outer window frame itself when clicking near the top
    TitleBar.InputBegan:Connect(function() end)
    ...
    -- Build separate resize handle (in HandleGui, not clipped)
    BuildResizeHandle(Outer, 420, 340)
...
-- UNIVERSAL COMPAT
--=============================================================================
local _LuaX_LibraryKey = "_LuaX_Linoria_Active"
if getgenv()[_LuaX_LibraryKey] and getgenv()[_LuaX_LibraryKey].ScreenGui then
    pcall(function()
        if getgenv()[_LuaX_LibraryKey].Unload then getgenv()[_LuaX_LibraryKey]:Unload() end
        getgenv()[_LuaX_LibraryKey].ScreenGui:Destroy()
    end)
end
getgenv()[_LuaX_LibraryKey] = Library
if not getgenv().Library or getgenv().Library == nil then
    getgenv().Library = Library
end

--=============================================================================
-- LIVE STUD UI (redesigned)
--=============================================================================
local function _LuaX_BuildStud()
    if Library._StudGui then return end
    ...
    outer.Name = "Stud"
    outer.Size = UDim2.new(0, 260, 0, 130)
    outer.Position = UDim2.new(1, -280, 0, 20)
    outer.BackgroundColor3 = Library.BackgroundColor
    outer.BackgroundTransparency = Library.GlassEnabled and 0.25 or 0.05
    outer.BorderSizePixel = 0
    outer.Active = true
    outer.Parent = sg
    Library:Create("UICorner", { CornerRadius = UDim.new(0, 8), Parent = outer })
    local border = Library:Create("UIStroke", { Color = Library.OutlineColor, Transparency = 0.3, Thickness = 1, Parent = outer })
    ...
    Library:Create("UICorner", { CornerRadius = UDim.new(0, 8), Parent = accentTop })
    ...
    local posLbl = Library:CreateLabel({
    ...
    -- X / Y / Z rows: label on left, value on right, fill bar behind
    local rowY = 44
    local function makeRow(letter, color)
        ...
        Library:Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = rowFrame })
        ...
        local letterLbl = Library:CreateLabel({
            Position = UDim2.new(0, 6, 0, 0),
    ...
    local xFill, xVal = makeRow("X", Color3.fromRGB(230, 90, 90))
    local yFill, yVal = makeRow("Y", Color3.fromRGB(90, 220, 130))
    local zFill, zVal = makeRow("Z", Color3.fromRGB(90, 140, 240))

    local function updateOuterSize()
        outer.Size = UDim2.new(0, 260, 0, rowY + 6)
    end
    updateOuterSize()
    ...
    RS.Heartbeat:Connect(function(dt)
        ...
        dot.BackgroundColor3 = Library.AccentColor:Lerp(Library.FontColor, p * 0.5)
        ...
        accentTop.BackgroundColor3 = Library.AccentColor
        border.Color = Library.OutlineColor
        header.TextColor3 = Library.FontColor
        posLbl.TextColor3 = Library.FontColor

        local c = LP.Character
        local hrp = c and c:FindFirstChild("HumanoidRootPart")
        if hrp then
            local pos = hrp.Position
            posLbl.Text = fmt(pos.X) .. ", " .. fmt(pos.Y) .. ", " .. fmt(pos.Z)
            xVal.Text = fmt(pos.X)
            yVal.Text = fmt(pos.Y)
            zVal.Text = fmt(pos.Z)
            xFill.Size = UDim2.new(math.clamp(math.abs(pos.X) / 500, 0, 1), 0, 1, 0)
            yFill.Size = UDim2.new(math.clamp(math.abs(pos.Y) / 500, 0, 1), 0, 1, 0)
            zFill.Size = UDim2.new(math.clamp(math.abs(pos.Z) / 500, 0, 1), 0, 1, 0)
        else
            posLbl.Text = "--, --, --"
            xVal.Text = "--"
            yVal.Text = "--"
...
-- AUTO EXECUTE
--=============================================================================
function Library:SetAutoExecute(bool, path)
    Library.AutoExecute.Enabled = bool
    if type(path) == "string" then Library.AutoExecute.Path = path end
    if bool then
        if Library._AutoExecConn then Library._AutoExecConn:Disconnect() end
        Library._AutoExecConn = LocalPlayer.CharacterAdded:Connect(function()
            task.wait(2)
            if Library.AutoExecute.Enabled and readfile and isfile and isfile(Library.AutoExecute.Path) then
                pcall(function() loadstring(readfile(Library.AutoExecute.Path))() end)
            end
        end)
        task.spawn(function()
            if readfile and isfile and isfile(Library.AutoExecute.Path) then
                pcall(function() loadstring(readfile(Library.AutoExecute.Path))() end)
            end
        end)
    else
        if Library._AutoExecConn then Library._AutoExecConn:Disconnect(); Library._AutoExecConn = nil end
    end
end

function Library:SaveAutoExecute(scriptSource)
    if not writefile then return false end
    pcall(function()
        if not isfolder("LuaXBeta") then makefolder("LuaXBeta") end
        writefile(Library.AutoExecute.Path, scriptSource)
    end)
    return true
end
...
Library.FontRegistry = {
    "Code", "Gotham", "GothamMedium", "GothamBold", "GothamBlack",
    "Arial", "ArialBold", "SourceSans", "SourceSansBold", "SourceSansLight",
    "SourceSansItalic", "Bodoni", "Garamond", "Cartoon", "Highway",
    "SciFi", "Arcade", "Fantasy", "Antique", "Legacy",
    "Roboto", "RobotoCondensed", "RobotoMono", "Ubuntu",
    "Nunito", "NunitoBold", "BuilderSans", "BuilderSansBold", "BuilderSansMedium",
}

function Library:ApplyFont(fontName)
    local ok = pcall(function() return Enum.Font[fontName] end)
    if not ok then return false end
    local target = Enum.Font[fontName]
    local function walk(inst)
        if not inst then return end
        if inst:IsA("TextLabel") or inst:IsA("TextButton") or inst:IsA("TextBox") then
            pcall(function() inst.Font = target end)
        end
        for _, child in ipairs(inst:GetChildren()) do walk(child) end
    end
    walk(Library.ScreenGui)
    walk(Library.Watermark)
    walk(Library.KeybindFrame)
    if Library._StudGui then walk(Library._StudGui) end
    Library.CurrentFont = target
    return true
end
...
if InputService.TouchEnabled then
    ...
    local function CreateMobileButton(name, text, startPos)
        local Outer = Library:Create('Frame', {
            Name = name .. "Outer", BackgroundColor3 = Library.OutlineColor, BorderSizePixel = 0,
            Position = startPos, Size = UDim2.new(0, BTN_W, 0, BTN_H), ZIndex = 300, Parent = MobileGui, Active = true,
        })
        Library:AddToRegistry(Outer, { BackgroundColor3 = 'OutlineColor' })
        local AccentFrame = Library:Create('Frame', {
            Name = name .. "Accent", BackgroundColor3 = Library.AccentColor, BorderSizePixel = 0,
            Position = UDim2.new(0, 1, 0, 1), Size = UDim2.new(1, -2, 1, -2), ZIndex = 301, Parent = Outer,
        })
        Library:AddToRegistry(AccentFrame, { BackgroundColor3 = 'AccentColor' })
        local Inner = Library:Create('Frame', {
            Name = name .. "Inner", BackgroundColor3 = Color3.fromRGB(8, 8, 12), BorderSizePixel = 0,
            Position = UDim2.new(0, 1, 0, 1), Size = UDim2.new(1, -2, 1, -2), ZIndex = 302, Parent = AccentFrame,
        })
        local Btn = Library:Create('TextButton', {
            Name = name .. "Btn", BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Font = Enum.Font.Code,
            Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextSize = Library.FontSize - 1,
            ZIndex = 304, Parent = Inner, Active = true,
        })
        return Outer, Btn
    end
