...
local SaveManager = {} do
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        assert(tab, 'SaveManager:BuildConfigSection -> tab is nil')
        ...
        section:AddButton('Create config', function()
            ...
            if not name or name:gsub(' ', '') == '' then
                self.Library:Notify('Config name is empty', 2)
                return
            end
            local success, err = self:Save(name)
            if not success then
                self.Library:Notify('Create failed: ' .. tostring(err))
                return
            end
        ...
        section:AddButton('Load config', function()
            ...
            local success, err = self:Load(name)
            if not success then
                self.Library:Notify('Load failed: ' .. tostring(err))
                return
            end
        ...
        section:AddButton('Save config', function()
            ...
            if not name or name == '' then
                self.Library:Notify('Pick or type a config name', 2)
                return
            end
            local success, err = self:Save(name)
            if not success then
                self.Library:Notify('Save failed: ' .. tostring(err))
                return
            end
        ...
        section:AddButton('Set as autoload', function()
            ...
            if not success then
                self.Library:Notify('Autoload failed: ' .. tostring(err))
                return
            end
            if self.AutoloadConfigLabel then
                self.AutoloadConfigLabel:SetText('Current autoload config: ' .. name)
            end
            self.Library:Notify('Autoload = "' .. name .. '"')
        end)

        section:AddButton('Reset autoload', function()
            local success, err = self:DeleteAutoLoadConfig()
            if not success then
                self.Library:Notify('Reset failed: ' .. tostring(err))
                return
            end
            if self.AutoloadConfigLabel then
                self.AutoloadConfigLabel:SetText('Current autoload config: none')
            end
            self.Library:Notify('Autoload cleared')
        end)

        self.AutoloadConfigLabel = section:AddLabel('Current autoload config: ' .. tostring(self:GetAutoloadConfig()), true)

        self:SetIgnoreIndexes({ 'SaveManager_ConfigList', 'SaveManager_ConfigName' })
        pcall(function() self.Library:Notify('Configs panel ready') end)
    end
