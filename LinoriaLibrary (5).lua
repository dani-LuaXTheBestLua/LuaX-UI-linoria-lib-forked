...
function Library:MakeResizable(Instance, MinSize)
    -- Better resize grip: small corner plate + 3 clear diagonal lines
    Instance.Active = true
    ...
    Library:AddToRegistry(Resizer, { BackgroundColor3 = "MainColor"; })

    -- 3 diagonal grip lines
    for i = 0, 2 do
        local Line = Library:Create("Frame", {
            BackgroundColor3 = Library.AccentColor;
            BackgroundTransparency = 0.1;
            BorderSizePixel = 0;
            Size = UDim2.fromOffset(12 - i * 2, 2);
            Position = UDim2.fromOffset(4 + i, 12 - i * 3);
            Rotation = -40;
            ZIndex = 21;
            Parent = Resizer;
        })
    ...
    Resizer.InputEnded:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1
            or Input.UserInputType == Enum.UserInputType.Touch then
            Dragging = false
            Resizer.BackgroundTransparency = 0.35
        end
    end)
    ...
    Library:GiveSignal(InputService.InputChanged:Connect(function(Input)
        ...
        Instance.Size = UDim2.fromOffset(
            math.clamp(StartSize.X + Delta.X, MinSize.X, 1e5),
            math.clamp(StartSize.Y + Delta.Y, MinSize.Y, 1e5)
        )
    end))
end
    ...
    function BaseGroupboxFuncs:AddViewport(Idx, Info)
        ...
        ViewportFrame.InputChanged:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.MouseWheel then
                ...
                Viewport.Camera.CFrame  = REPLACE_PLUS_EQ  Viewport.Camera.CFrame.LookVector * ZoomAmount
            end
        end)
        ...
        Library:GiveSignal(InputService.TouchPinch:Connect(function(touchPositions, scale, velocity, state)
            ...
            elseif state == Enum.UserInputState.Change then
                ...
                Viewport.Camera.CFrame  = REPLACE_PLUS_EQ  Viewport.Camera.CFrame.LookVector * delta
