...
function Library:MakeResizable(Frame, MinSize)
    -- Classic Linoria-style resize: ONLY diagonal lines, no filled blob
    ...
    Hit.Size = UDim2.fromOffset(S + 10, S + 10)
    Hit.Position = UDim2.new(1, -(S + 10), 1, -(S + 10))
    Hit.Text = ''
    Hit.ZIndex = 250
    Hit.AutoButtonColor = false
    Hit.Parent = Frame

    -- Lines only (no filled square / no circle)
    local Lines = {}
    local function MakeLine(i, color, trans)
        local Line = Instance.new('Frame')
        Line.BackgroundColor3 = color
        Line.BackgroundTransparency = trans
        Line.BorderSizePixel = 0
        Line.Size = UDim2.fromOffset(12 - i * 3, 2)
        Line.Position = UDim2.fromOffset(4 + i * 2, 14 - i * 4)
        Line.Rotation = -45
        Line.ZIndex = 251
        Line.Parent = Hit
        return Line
    ...
    for i = 0, 2 do
        Lines[i + 1] = MakeLine(i, Library.AccentColor, 0.25)
        Library:AddToRegistry(Lines[i + 1], { BackgroundColor3 = 'AccentColor' })
    end
    ...
    local function SetHover(on)
        local col = on and Library:GetDarkerColor(Library.AccentColor) or Library.AccentColor
        local tr = on and 0 or 0.25
    ...
    Hit.MouseLeave:Connect(function() if not Dragging then SetHover(false) end end)
    ...
    Library:GiveSignal(InputService.InputChanged:Connect(function(Input)
        ...
        Frame.Size = UDim2.fromOffset(
            math.clamp(StartSize.X + Delta.X, MinSize.X, 2500),
            math.clamp(StartSize.Y + Delta.Y, MinSize.Y, 2500)
        )
    end))
end
...
function Library:CreateWindow(...)
    ...
    -- resize grip created below (no blob)
    ...
    -- Classic resize grip: 3 diagonal lines ONLY (no circle blob)
    do
        local MinW, MinH = 420, 340
        ...
        for i = 0, 2 do
            local Line = Library:Create('Frame', {
                ...
                BorderSizePixel = 0;
                Size = UDim2.fromOffset(12 - i * 3, 2);
                Position = UDim2.fromOffset(5 + i * 2, 14 - i * 4);
                Rotation = -45;
                ZIndex = 251;
                Parent = Hit;
            });
        ...
        local function SetHover(on)
            local c = on and Library:GetDarkerColor(Library.AccentColor) or Library.AccentColor
            local t = on and 0 or 0.2
            for _, L in ipairs(Lines) do
                L.BackgroundColor3 = c
                L.BackgroundTransparency = t
            end
        end

        local Resizing, StartSize, DragStart, DragType
        ...
        Hit.MouseEnter:Connect(function() if not Resizing then SetHover(true) end end)
        Hit.MouseLeave:Connect(function() if not Resizing then SetHover(false) end end)
        ...
        Hit.InputBegan:Connect(function(Input)
            ...
            ResizeConn = InputService.InputChanged:Connect(function(Change)
                ...
                local VP = Library.ScreenGui.AbsoluteSize
                local NewW = math.clamp(StartSize.X.Offset + Delta.X, MinW, VP.X - TopLeft.X)
                local NewH = math.clamp(StartSize.Y.Offset + Delta.Y, MinH, VP.Y - TopLeft.Y)
